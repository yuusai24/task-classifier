  ...
  return (
    <div>
      ...
      <ul className="flex flex-col gap-3">
        {(bookings ?? []).map((b) => (
          <li key={b.id} className="rounded-lg border border-purple-200/60 bg-white/60 p-3 text-sm">
            ...
            {b.note && <p className="mt-1 text-zinc-500">メモ: {b.note}</p>}
            <form action={updateBooking.bind(null, b.id)} className="mt-2 flex flex-wrap items-center gap-2">
              ...
              />
              <button type="submit" className="rounded-md bg-gradient-to-r from-purple-400 to-pink-400 px-3 py-1 text-xs text-white transition hover:brightness-110">
                保存
              </button>
            </form>
          </li>
