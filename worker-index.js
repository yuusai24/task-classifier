...
.stats{display:grid;grid-template-columns:repeat(2,1fr);background:white;border-bottom:1px solid #e0e0e0}
...
function renderList(){
  ...
  document.getElementById('litems').innerHTML=students.map((s,i)=>
    ...
    '<div class="sinfo"><div class="sname">'+s.name+statusBadge(s.status)+'</div></div>'+
    '</div>'
  ).join('');
}
function sel(id){
  ...
  document.getElementById('detail').innerHTML=
    ...
    '<div><div class="dname">'+s.name+'</div><div class="dmeta">入会 '+s.joined_at+'</div></div>'+
    '<div class="dbtns">'+
...
async function save(id){
  ...
  await fetch('/api/students/'+encodeURIComponent(id),{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({notes,status})});
  const s=students.find(x=>x.id===id);
  s.notes=notes;s.status=status;
...
function openAddStudent(){
  ['a-id','a-name','a-joined','a-job','a-notes'].forEach(id=>document.getElementById(id).value='');
...
async function addStudent(){
  ...
  const body={id,name,joined_at:document.getElementById('a-joined').value.trim(),job:document.getElementById('a-job').value.trim(),notes:document.getElementById('a-notes').value.trim(),status:'active'};
  await fetch('/api/students',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
  closeAddModal();await load();alert(name+'を追加したよ！');
}
function openBulkSend(){
  ...
  showBulkStep1();document.getElementById('bulk-modal').classList.add('open');
}
function closeBulkModal(){document.getElementById('bulk-modal').classList.remove('open');}
function showBulkStep1(){document.getElementById('bulk-step1').style.display='block';document.getElementById('bulk-step2').style.display='none';}
...
async function executeBulkSend(){
  ...
  const data=await res.json();
  closeBulkModal();alert('送信完了！'+data.count+'名に送ったよ。');
...
async function openPreview(id){
  ...
  document.getElementById('modal').classList.add('open');await loadPreview();
}
...
function setType(btn){
  ...
  btn.classList.add('active');loadPreview();
}
...
async function sendFromModal(){
  ...
  closeModal();alert('送信しました！');loadHistory(modalStudentId);
}
...
async function load(){
  ...
  renderStats();renderList();loadMessages();
}
load();
<\/script>
</body>
  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/webhook" && request.method === "POST") {
      ...
      for (const event of body.events || []) {
        if (event.type === "message" && event.message.type === "text") {
          ...
          await env.DB.prepare("INSERT INTO line_messages (student_id, student_name, message, received_at) VALUES (?, ?, ?, ?)").bind(userId, student?.name || null, text, (new Date()).toISOString()).run();
        }
      }
    ...
    if (url.pathname === "/api/students") {
      ...
      if (request.method === "POST") {
        const body = await request.json();
        await env.DB.prepare("INSERT INTO students (id, name, joined_at, current_stage, progress, job, notes, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)").bind(body.id, body.name, body.joined_at, body.current_stage || null, body.progress || 0, body.job, body.notes, body.status || "active").run();
    ...
    if (url.pathname.startsWith("/api/students/")) {
      ...
      if (request.method === "PUT") {
        const body = await request.json();
        await env.DB.prepare("UPDATE students SET notes=?, status=? WHERE id=?").bind(body.notes, body.status || "active", id).run();
        return Response.json({ ok: true }, { headers: cors });
      }
    ...
    if (url.pathname === "/api/send-all" && request.method === "POST") {
      ...
      if (body.messages && body.messages.length) {
        for (const m of body.messages) {
          ...
          await env.DB.prepare("INSERT INTO feedback_history (student_id, message_type, message, sent_at) VALUES (?, ?, ?, ?)").bind(m.id, m.type || messageType, m.message, (new Date()).toISOString()).run();
          count++;
        }
      } else {
        ...
        for (const student of result.results) {
          const replyRow = await env.DB.prepare("SELECT group_concat(message, '\n---\n') as message FROM (SELECT message FROM line_messages WHERE student_id=? ORDER BY received_at ASC LIMIT 10)").bind(student.id).first();
          ...
          await env.DB.prepare("INSERT INTO feedback_history (student_id, message_type, message, sent_at) VALUES (?, ?, ?, ?)").bind(student.id, messageType, feedback, (new Date()).toISOString()).run();
          count++;
        }
      }
    ...
    if (url.pathname.startsWith("/api/preview/")) {
      ...
      const replyRow = await env.DB.prepare("SELECT group_concat(message, '\n---\n') as message FROM (SELECT message FROM line_messages WHERE student_id=? ORDER BY received_at ASC LIMIT 10)").bind(id).first();
    ...
    if (url.pathname.startsWith("/api/send-message/")) {
      ...
      await env.DB.prepare("INSERT INTO feedback_history (student_id, message_type, message, sent_at) VALUES (?, ?, ?, ?)").bind(id, body.type || "manual", body.message, (new Date()).toISOString()).run();
      return Response.json({ ok: true }, { headers: cors });
    }
    if (url.pathname.startsWith("/api/send/")) {
      ...
      if (!student) return Response.json({ error: "not found" }, { status: 404, headers: cors });
      const replyRow = await env.DB.prepare("SELECT group_concat(message, '\n---\n') as message FROM (SELECT message FROM line_messages WHERE student_id=? ORDER BY received_at ASC LIMIT 10)").bind(id).first();
      const feedback = await generateFeedback(student, "monday", env.ANTHROPIC_API_KEY, replyRow?.message || null);
      await sendLine(student.id, feedback, env.LINE_CHANNEL_ACCESS_TOKEN);
      await env.DB.prepare("INSERT INTO feedback_history (student_id, message_type, message, sent_at) VALUES (?, ?, ?, ?)").bind(id, "monday", feedback, (new Date()).toISOString()).run();
      return Response.json({ ok: true }, { headers: cors });
    }
    if (url.pathname.startsWith("/api/history/")) {
      const id = decodeURIComponent(url.pathname.split("/")[3]);
      const result = await env.DB.prepare("SELECT * FROM feedback_history WHERE student_id=? ORDER BY sent_at DESC LIMIT 20").bind(id).all();
      return Response.json(result.results, { headers: cors });
    }
    if (url.pathname === "/api/messages") {
      const result = await env.DB.prepare("SELECT * FROM line_messages ORDER BY received_at DESC LIMIT 20").all();
      return Response.json(result.results, { headers: cors });
    }
...
async function generateFeedback(student, messageType, apiKey, latestReply) {
  const typePrompts = {
    monday: "月曜日の元気あふれるメッセージを送ります。①「素晴らしい１週間が来たよ！」という書き出しで始める。②「今週は何をやめる？」「今週は何をする？」という2つの問いかけを含める。③「守護霊がバックアップするよ！」のような締めの言葉で終わる。受講生の仕事・状況を踏まえた具体的な内容にしてください。",
    wednesday: "水曜日の中間チェックインメッセージを送ります。今どんな感じ？という雰囲気で、今週の半分を過ごした受講生に寄り添いながら、今困っていることや行き詰まっていることがあれば教えてほしい、と自然に投げかける内容にしてください。",
    friday: "金曜日の週間お疲れさまメッセージを送ります。今週1週間を労い、受講生の成長を認め、週末をゆっくり過ごしてほしいという温かい内容にしてください。",
    monthly: "月末の振り返りレポートをお願いするメッセージを送ります。以下の5項目を返信してもらえるようにお願いしてください。温かく、負担にならない雰囲気で伝えてください。①うまくいったこと ②うまくいかなかったこと ③今月の成果（売り上げはもちろん、旦那さんにハグしてもらった、子どもにありがとうと言い合えた、受講生から感謝された、など受け取ったものすべて） ④今月気づいたこと ⑤来月実行すること",
  ...
  const prompt = "あなたは女性起業家・副業ママのビジネスコーチ「友彩（ゆうさい）」のAIアシスタントです。受講生へのLINEメッセージを作成してください。【ルール】200〜300文字以内。ためぐちで書く（です・ます調は使わない）。関西弁を必ず混ぜる（やん・やで・やね・やんか・めっちゃ・ほんま など。ただしここここにしすぎない）。受講生の仕事・状況を具体的に言及する。個人の家族状況などは文中に書かない。締めは温かい言葉で終わる。【メッセージの種類】" + typePrompts[messageType] + "【受講生情報】仮名: " + student.name + " 入会日: " + student.joined_at + " 仕事・肩書き: " + student.job + " 最近の状況: " + student.notes + " 受講生の過去の返信履歴（直近10件）: " + (latestReply || "なし");
